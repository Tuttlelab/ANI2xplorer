...
class BinlessWHAM:
    def __init__(self, distances, bias_energies, r0_values, kappa, temperature, max_iterations=10000, tolerance=1e-8):
    ...
    def run_wham(self, n_points=200):
            ...
            bw_factors = np.clip(1 / np.sqrt(n_samples), 0.1, 0.5)  # Avoid overfitting
            for i, dists in enumerate(self.distances):
                kde = gaussian_kde(dists, bw_method=bw_factors[i])
                kde_list.append(kde)
    
            kde_values = np.array([kde(reaction_coords) for kde in kde_list])  # (n_windows, n_points)
    
            # Bias energy evaluation at selected reaction coordinates
            bias_at_x = np.array([
                0.5 * (bias_k[0] * (reaction_coords - bias_k[1])**2)  # Harmonic bias U = 0.5*k*(x-x0)^2
            ...
            for iteration in range(self.max_iterations):
                ...
                exp_weights = np.exp(-self.beta * (bias_at_x - F_k[:, np.newaxis]))  # Shape: (n_windows, n_points)
                denominator = np.sum(kde_values * exp_weights, axis=0)  # Shape: (n_points,)
                ...
                F_k = (1 - adaptive_weight) * F_k_old + adaptive_weight * (-1/self.beta * np.log(P_i.sum(axis=0)))
...
if __name__ == "__main__":
    ...
    Run = 15
    kappa = 50.0
    Reaction = "Reaction_1C"
    n_windows = 30
    temperature = 300
    
    distances, bias_energies = read_dat_file(f"Working_Folder/ANI/{Reaction}/{Run}/*/COLVAR*.dat")
    
    # r0_range=(1.02, 1.97, 2.38, 1.81) # Reaction 1A
    # r0_range=(1.34, 3.43, 4.44, 1.34) # Reaction 2A
    r0_range=(2.88,1.34,4.4448,1.79) # Reaction 1C
    # r0_range=(1.34,2.88,1.79,4.4448) # Reaction 1C Backward
    # r0_range=(3.54, 1.36, 2.59, 1.34)
