...
def print_window_avg_conf(path_to_highest_window_traj, target_directory):
    ...
    with open(f"{target_directory}/avg_positions_conf.xyz", "w") as f:
...
def get_highest_window(x):
    ...
    for window in tqdm.tqdm(energies):
        ...
        mean_window_E, position_min_window_conf = get_bin_Es(window)

        positions_of_min_window_conf.append(position_min_window_conf)
        mean_window_Es.append(mean_window_E)

    positions_of_min_window_conf = np.array(
        positions_of_min_window_conf
    )  # positions_of_min_window_conformers
    mean_window_Es = np.array(mean_window_Es)  # all windows mean Es

    print(
        "Bin with highest mean E is:  Window ",
        str(int(mean_window_Es.argmax() + 1)),
    )

    highest_window = int(
        mean_window_Es.argmax() + 1
    )  # numpy indexing starts from 0, but windows start from 1

    lowest_E_conf = positions_of_min_window_conf[mean_window_Es.argmax()]

    print(
        f"The lowest energy conformer of Window {highest_window} is at postion {lowest_E_conf}"
    )


if __name__ == "__main__":

    get_highest_window(1)
